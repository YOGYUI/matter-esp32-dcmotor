...
CMotorPWMDriver::CMotorPWMDriver()
{
    ...
    m_pwm_frequency_hz = 25000;  // 25 KHz
    m_pwm_count_tick = 1;
}
...
bool CMotorPWMDriver::create_mcpwm_timer()
{
    ...
    config.intr_priority = 0;  // MCPWM timer interrupt priority, if set to 0, the driver will try to allocate an interrupt with a relative low priority (1,2,3)
    // config.flags.update_period_on_empty;  // Whether to update period when timer counts to zero
    // config.flags.update_period_on_sync;  // Whether to update period on sync event
...
bool CMotorPWMDriver::create_mcpwm_operator()
{
    ...
    config.intr_priority = 0;       // MCPWM operator interrupt priority, if set to 0, the driver will try to allocate an interrupt with a relative low priority (1,2,3)
    // config.flags.update_gen_action_on_tez;   // Whether to update generator action when timer counts to zero
    // config.flags.update_gen_action_on_tep;   // Whether to update generator action when timer counts to peak
    // config.flags.update_gen_action_on_sync;  // Whether to update generator action on sync event
    // config.flags.update_dead_time_on_tez;    // Whether to update dead time when timer counts to zero
    // config.flags.update_dead_time_on_tep;    // Whether to update dead time when timer counts to peak
    // config.flags.update_dead_time_on_sync;   // Whether to update dead time on sync event
...
bool CMotorPWMDriver::create_mcpwm_comparator()
{
    ...
    config.intr_priority = 0;   // MCPWM comparator interrupt priority, if set to 0, the driver will try to allocate an interrupt with a relative low priority (1,2,3)
    config.flags.update_cmp_on_tez = true;  // Whether to update compare value when timer count equals to zero (tez)
    // config.flags.update_cmp_on_tep       // Whether to update compare value when timer count equals to peak (tep)
    // config.flags.update_cmp_on_sync      // Whether to update compare value on sync event
    ...
    mcpwm_comparator_set_compare_value(m_mcpwm_comparator, m_pwm_count_tick);
...
bool CMotorPWMDriver::create_mcpwm_generator()
{
    ...
    // config.flags.invert_pwm;     // Whether to invert the PWM signal (done by GPIO matrix)
    // config.flags.io_loop_back;   // For debug/test, the signal output from the GPIO will be fed to the input path as well
    // config.flags.io_od_mode;     // Configure the GPIO as open-drain mode
...
bool CMotorPWMDriver::set_duty_ratio(float ratio)
{
    ...
    uint32_t tick = m_pwm_count_tick - (uint32_t)((float)m_pwm_count_tick * ratio / 100.f);
