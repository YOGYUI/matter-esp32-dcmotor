...
bool CSystem::matter_update_attribute(uint16_t endpoint_id, uint32_t cluster_id, uint32_t attribute_id, esp_matter_attr_val_t *value)
{
    std::string strvalue = get_matter_attribute_value_string(*value);
    GetLogger(eLogType::Info)->Log("Try to update matter attribute (endpoint: %d, cluster: 0x%04X, attribute: 0x%04X, value: %s)", 
        endpoint_id, cluster_id, attribute_id, strvalue.c_str());
    esp_err_t ret = esp_matter::attribute::update(endpoint_id, cluster_id, attribute_id, value);
    if (ret != ESP_OK) {
